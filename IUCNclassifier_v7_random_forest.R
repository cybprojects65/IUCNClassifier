...
#####RF parameters
crossvalidate <- F  # Do cross-validation (T/F)
...
#Reference.data <- read.csv("AAA_RF_IUCN_NoBlanks240116.csv")
...
names(training.data.important)[which(names(training.data.important) == targetcolumnset.orig[1])]<-"o1"
names(training.data.important)[which(names(training.data.important) == targetcolumnset.orig[2])]<-"o2"
